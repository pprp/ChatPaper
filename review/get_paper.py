            ...
            for block in blocks:
                ...
                if found_abstract:
                    ...
                    for line in lines:
                        for span in line['spans']:
                            # 如果当前文本是子标题
                            if not font_heading and span['text'].isupper(
                            ) and sum(
                                    1 for c in span['text'] if c.isupper() and
                                ('A' <= c <= 'Z')) > 4:  # 针对一些标题大小一样,但是全大写的论文
                                upper_heading = True
                            ...
                            if not upper_heading and span[
                                    'size'] > threshold and re.match(  # 正常情况下,通过字体大小判断
                                        r'[A-Z][a-z]+(?:\s[A-Z][a-z]+)*',
                                        span['text'].strip()):
                                font_heading = True
                                if heading_font == -1:
                            ...
                            # 否则将当前文本添加到上一个子标题的文本中
                                ...
                                section_dict[last_heading] += ' ' + span[
                                    'text'].strip()
        self.section_names = subheadings
        self.section_texts = section_dict
