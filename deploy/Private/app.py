...
class Reader:
    ...
    def __init__(self,
        ...
        self.chatPaper = chatPaper(api_keys=self.chat_api_list,
                                   apiTimeInterval=10)
        self.chat_api_list = [
            api.strip() for api in self.chat_api_list if len(api) > 5
        ]
        self.cur_api = 0
    ...
    def summary_with_chat(self, paper_list):
        ...
        for paper_index, paper in enumerate(paper_list):
            ...
            if first_image is None or self.gitee_key == '':
                ...
                image_url = self.upload_gitee(image_path=first_image,
    ...
    def chat_conclusion(self, text):
        self.chatPaper.reset(convo_id='chatConclusion',
                             system_prompt='你是一个[' + self.key_word +
                             ']领域的审稿人，你需要严格评审这篇文章')
        self.chatPaper.add_to_conversation(
            convo_id='chatConclusion',
            role='assistant',
            message=str(
                '这是一篇英文文献的<summary>和<conclusion>部分内容，其中<summary>你已经总结好了，但是<conclusion>部分，我需要你帮忙归纳下面问题：'
                + text))
        content = """
                 8. 做出如下总结：
                    - (1):这篇工作的意义如何？
    ...
    def chat_method(self, text):
        self.chatPaper.reset(convo_id='chatMethod',
                             system_prompt='你是一个[' + self.key_word +
        ...
        self.chatPaper.add_to_conversation(
            ...
            role='assistant',
            message=str(
                '这是一篇英文文献的<summary>和<Method>部分内容，其中<summary>你已经总结好了，但是<Methods>部分，我需要你帮忙阅读并归纳下面问题：'
                + text))
        content = """
    ...
    def chat_summary(self, text):
        self.chatPaper.reset(convo_id='chatSummary',
                             system_prompt='你是一个[' + self.key_word +
                             ']领域的科研人员，善于使用精炼的语句总结论文')
        self.chatPaper.add_to_conversation(
            ...
            role='assistant',
            message=str(
                '这是一篇英文文献的标题，作者，链接，Abstract和Introduction部分内容，我需要你帮忙阅读并归纳下面问题：'
                + text))
        content = """
                 1. 标记出这篇文献的标题(加上中文翻译)
                 2. 列举所有的作者姓名 (使用英文)
...
interface = gradio.Interface(fn=upload_pdf,
                             inputs=ip,
                             outputs='html',
                             title=title,
