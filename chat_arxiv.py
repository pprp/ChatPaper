...
class Reader:
    ...
    def get_all_titles_from_web(self, keyword, page_num=1, days=1):
        ...
        for page in range(page_num):
            ...
            for title_index, title in enumerate(titles):  # 遍历每个标题，并打印出来
                print(page, title_index, title, links[title_index],
                      dates[title_index])
    ...
    def get_arxiv_web(self, args, page_num=1, days=2):
        titles, links, dates = self.get_all_titles_from_web(args.query,
                                                            page_num=page_num,
                                                            days=days)
        paper_list = []
    ...
    def download_pdf(self, url, title):
        ...
        path = self.root_path + 'pdf_files/' + self.validateTitle(
            self.args.query) + '-' + date_str
        try:
            os.makedirs(path)
        except:
            pass
        ...
        with open(filename,
                  'wb') as f:  # open a file with write and binary mode
        ...
        for paper_index, paper in enumerate(paper_list):
            ...
            try:
                ...
                if 'maximum context' in str(e):
                    current_tokens_index = str(e).find(
                        'your messages resulted in') + len(
                    ...
                    if 'maximum context' in str(e):
                        current_tokens_index = str(e).find(
                            'your messages resulted in') + len(
                                'your messages resulted in') + 1
                        offset = int(
                            str(e)[current_tokens_index:current_tokens_index +
                                   4])
            ...
            file_name = os.path.join(
                export_path, date_str + '-' + self.validateTitle(self.query) +
                '.' + self.file_format)
    ...
    def chat_conclusion(self, text, conclusion_prompt_token=800):
        ...
        self.cur_api += 1
        self.cur_api = 0 if self.cur_api >= len(
            self.chat_api_list) - 1 else self.cur_api
        ...
        messages = [
            {
                ...
                'system',
                'content':
                'You are a reviewer in the field of [' + self.key_word +
        ...
        messages = [
            {
                ...
                'system',
                'content':
                'You are a researcher in the field of [' + self.key_word +
...
def chat_arxiv_main(args):
    ...
    reader1.show_info()
    paper_list = reader1.get_arxiv_web(args=args,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--page_num',
                        type=int,
                        default=1,
                        help='the maximum number of page')
    ...
    parser.add_argument(
        ...
        default=False,
        help=
        'save image? It takes a minute or two to save a picture! But pretty')
